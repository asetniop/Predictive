...
function init(){
	...
	for(var i = 0; i < trigramkeys.length; i++){
		...
		else if(trigramdic[activeword].p){
			activering = trigramdic[activeword].p;
			preringassignments[activeword] = activeword;
			preringtotals[activeword] = activecount
			for(var j = 0; j < activering.length; j++){
				preringassignments[activering[j]] = activeword
				preringtotals[activeword] += wordcount(activering[j])
			}
			activering.push(activeword)
			prerings[activeword] = activering
		}
	...
	if(c == " "){
		...
		if(potentialword){
			...
			if(activeobject){
				...
				preringactive = 0;
				if(preringassignments[previousword]){
					presynpreviousword = preringassignments[previousword]
					preringarray = prerings[presynpreviousword]
					preringactive = 1;
				}
				...
				if(finalchoicearray.length < numberofchoices){
					for(k in basechoicearray){
						...
						if(extrasactive == 1){
							...
							//looks for presynonym connections
							preringboost = 1;
							if(preringactive == 1){
								for(j in preringarray){
									precheckword = preringarray[j]
									if (trigramdic[synbaseword] && trigramdic[synbaseword]['z'] && trigramdic[synbaseword]['z'][precheckword]){
										preringboost = presynboost
										if(tag == '-b'){
											tag = 'Xb'
										}
										else{
						...
						frequencyfactor = wordlengthfactor*firstextrafactor*linkboost*preringboost
