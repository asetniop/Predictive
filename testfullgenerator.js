...
var precorrelationthreshold = 1; //much more agressive
var nextoutsize = 15;
var outpower = 0.2;
var nextzero = 10;
var slope = 0.03;
...
//ringwordsarray = {}
	...
	sortedrings = ringgenerator(synonymspace, correlationthreshold)
	...
	/*
	presortedrings = ringgenerator(presynonymspace, precorrelationthreshold)
	prerings = presortedrings
	preringexport = {}
	//add prerings to priority file
	keys = Object.keys(prerings)
	for(var i = 0; i < keys.length; i++){
		preringword = keys[i]
		
		//remove key word; not needed in array
		activeprering = prerings[preringword];
		remove = activeprering.indexOf(preringword)
		activeprering.splice(remove, 1);
		
		preringexport[preringword] = activeprering
	}
	*/
...
function ringgenerator(functionring, corrthresh){


//GENERATE SYNONYM RINGS

	basewordsarray = Object.keys(functionring)
	allwordsarray = []
	for(var i = 0; i < basewordsarray.length; i++){
		baseword = basewordsarray[i]
		subwordsarray = Object.keys(functionring[baseword])
		for(var j = 0; j < subwordsarray.length; j++){
			subword = subwordsarray[j]
			if(wordspace[baseword] && wordspace[subword] && functionring[baseword][subword] > corrthresh){ //only passes on ones that are actually good matches and are above synonym floor threshold
				allwordsarray.push([baseword, subword, functionring[baseword][subword]])
			}
		}
	}
	...
	for(var i = 0; i < allwordsarray.length; i++){
		...
		if(assigned[word0] && assigned[word1]){
			...
			if(genrings[word0]){
				if(genrings[word0].length < ringlimit){
					genrings[word0].push(word1);
			...
			else if(genrings[word1]){
				if(genrings[word1].length < ringlimit){
					genrings[word1].push(word0);
	...
	if(orphanarray.length > 0){ //assign lead orphan name
		for(var i = 0; i < orphanarray.length; i++){
			...
			if(assigned[word0] && assigned[word1]){
				...
				if(genrings[word0]){
					if(genrings[word0].length < superringlimit){
						genrings[word0].push(word1);
						assigned[word1] = word0;
					}
				}
				else if(genrings[word1]){
					if(genrings[word1].length < superringlimit){
						genrings[word1].push(word0);
						assigned[word0] = word1;
					}
				}
	...
	originalringsarray = Object.keys(genrings)
	xsortedrings = {}
	for(i = 0; i < originalringsarray.length; i++) {
		ringcontents = genrings[originalringsarray[i]]
		target = 0;
		targetword = '';
		...
		xsortedrings[targetword] = ringcontents
	}
	return xsortedrings
}