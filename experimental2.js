...
var trigramdictionaryoutsize = ringdictionarysize; // total size of trigram output dictionary
...
fs.readFile(corpus, function(err, data) {
	...
	for(j = 0; j < ringwordsarray.length; j++) {
		...
		presynonymspace[ringwordsarray[j]] = {};
	}
...
for(i = 0; i < looparray.length; i++){
	...
	for(j = loopstart; j < loopend; j++) {
		...
		for(k = 0; k < wordsinline; k++) {
			if(k + 3 < wordsinline){
				...
				else if(presynonymspace[blankword]){
					quadline = linearray[k+1]+'_'+linearray[k+2]+'_'+linearray[k+3]
					prequadpackageA = quadcheck(prequadpackageA, quadline, blankword)
				}
			}
		}
	}
	...
	quadpackageA = {}
	presynonymtally(prequadpackageA)
	prequadpackageA = {}
		...
		for(k = 0; k < wordsinline; k++) {
			if(k > 0 && k + 2 < wordsinline){
				...
				if(synonymspace[blankword]){
					...
					quadpackageB = quadcheck(quadpackageB, quadline, blankword)
				}
				else if(presynonymspace[blankword]){
					quadline = linearray[k+0]+'_'+linearray[k+2]+'_'+linearray[k+3]
					prequadpackageB = quadcheck(prequadpackageB, quadline, blankword)
				}
			}
		}
	}
	synonymtally(quadpackageB)
	quadpackageB = {}
	presynonymtally(prequadpackageB)
	prequadpackageB = {}
		...
		for(k = 0; k < wordsinline; k++) {
			if(k > 1 && k + 1 < wordsinline){
				...
				if(synonymspace[blankword]){
					...
					quadpackageC = quadcheck(quadpackageC, quadline, blankword)
				}
				else if(presynonymspace[blankword]){
					quadline = linearray[k+0]+'_'+linearray[k+1]+'_'+linearray[k+3]
					prequadpackageC = quadcheck(prequadpackageC, quadline, blankword)
				}
			}
		}
	}
	synonymtally(quadpackageC)
	quadpackageC = {}
	presynonymtally(prequadpackageC)
	prequadpackageC = {}
		...
		for(k = 0; k < wordsinline; k++) {
			if(k > 2 && k < wordsinline){
				...
				if(synonymspace[blankword]){
					...
					quadpackageD = quadcheck(quadpackageD, quadline, blankword)
				}
				else if(synonymspace[blankword]){
					quadline = linearray[k+0]+'_'+linearray[k+1]+'_'+linearray[k+2]
					prequadpackageD = quadcheck(prequadpackageD, quadline, blankword)
				}
			}
		}
	}
	synonymtally(quadpackageD)
	quadpackageD = {}
	presynonymtally(prequadpackageD)
	prequadpackageD = {}
	...
	keys = Object.keys(presynonymspace)
	for(j in keys){
		word = keys[j]
		presynonymspace[word] = synonymsorter(presynonymspace[word],maxarraylength)
	}
}
...
//deal with presynonymspace
keys = Object.keys(presynonymspace)
for(i in keys){
	word = keys[i]
	subkeys = Object.keys(presynonymspace[word])
	for(j in subkeys){
		subword = subkeys[j]
		if(ringwordspace[word] && ringwordspace[subword]){
			subcalc = synonymconstant*presynonymspace[word][subword]/(ringwordspace[word].n + ringwordspace[subword].n)
			presynonymspace[word][subword] = (Math.round(subcalc*100))/100
		}
		else{
			console.log(word + '-' + subword)
		}
	}
	presynonymspace[word] = synonymsorter(presynonymspace[word],synonymsmax)
}

longJSONfile(presynonymspace, 'presynonym')
presynonymring = presynonymspace;
...
//GENERATE SYNONYM RINGS
	...
	sortedrings = ringgenerator(synonymring, synonymfloor, 10000000)
	...
	presortedrings = ringaggregator(presynonymring)

	prerings = presortedrings

	preringexport = {}
	//add rings to priority file
	keys = Object.keys(prerings)
	for(var i = 0; i < keys.length; i++){
		ringword = keys[i]
		
		//remove key word; not needed in array
		activering = prerings[ringword];
		remove = activering.indexOf(ringword)
		activering.splice(remove, 1);
		
		preringexport[ringword] = activering
	}
...
//get "third" and "penultimate" objects
	...
	for(var i = 0; i < keys.length; i++){
		...
		else if(preringexport[trigramword]){
			trigramdic[trigramword]['p'] = preringexport[trigramword]
		}
	}
...
function presynonymtally(quadpackage){
	keys = Object.keys(quadpackage)
	for(m in keys){
		subpackage = quadpackage[keys[m]]
		subkeys = Object.keys(subpackage)
		...
		if(n > 1){
			for(n in subkeys){
				...
				for(o in subkeys){
					if(subkeys[o] != word){
						if(presynonymspace[word][subkeys[o]]){
							presynonymspace[word][subkeys[o]] += 1
						}
						else{
							presynonymspace[word][subkeys[o]] = 1
						}
...
function ringgenerator(activering, rangebottom, rangetop){

	basewordsarray = Object.keys(activering)
	allwordsarray = []
	for(var i = 0; i < basewordsarray.length; i++){
		baseword = basewordsarray[i]
		subwordsarray = Object.keys(activering[baseword])
		for(var j = 0; j < subwordsarray.length; j++){
			subword = subwordsarray[j]
			if(wordspace[baseword] && wordspace[subword] && activering[baseword][subword] > correlationthreshold && sortedwordsarray.indexOf(baseword) >= rangebottom && sortedwordsarray.indexOf(baseword) < rangetop){ //only passes on ones that are actually good matches and are above synonym floor threshold
				allwordsarray.push([baseword, subword, activering[baseword][subword]])
			}
		}
	}
	...
	for(var i = 0; i < allwordsarray.length; i++){
		...
		if(assigned[word0] && assigned[word1]){
			...
			if(functionrings[word0]){
				if(functionrings[word0].length < ringlimit){
					functionrings[word0].push(word1);
			...
			else if(functionrings[word1]){
				if(functionrings[word1].length < ringlimit){
					functionrings[word1].push(word0);
			...
			assigned[word1] = word0;
			functionrings[word0] = []
			functionrings[word0].push(word0);
			functionrings[word0].push(word1);
		}
	...
	if(orphanarray.length > 0){ //assign lead orphan name
		for(var i = 0; i < orphanarray.length; i++){
			...
			if(assigned[word0] && assigned[word1]){
				...
				if(functionrings[word0]){
					if(functionrings[word0].length < superringlimit){
						functionrings[word0].push(word1);
						assigned[word1] = word0;
					}
				}
				else if(functionrings[word1]){
					if(functionrings[word1].length < superringlimit){
						functionrings[word1].push(word0);
						assigned[word0] = word1;
					}
				}
	...
	originalringsarray = Object.keys(functionrings)
	sortedrings = {}
	for(i = 0; i < originalringsarray.length; i++) {
		ringcontents = functionrings[originalringsarray[i]]
		target = 0;
		targetword = '';
...
function ringaggregator(activering){
	...
	for(var i = 0; i < basewordsarray.length; i++){
		...
		for(var j = 0; j < subwordsarray.length; j++){
			subword = subwordsarray[j]
			if(wordspace[baseword] && wordspace[subword]){ //only passes on ones that are actually good matches and are above synonym floor threshold
				allwordsarray.push([baseword, subword, activering[baseword][subword]])
			}
		}
	}
	...
	for(var i = 0; i < allwordsarray.length; i++){
		...
		if(assigned[word0] && assigned[word1]){
			...
			if(functionrings[assigned[word0]]){
				functionrings[assigned[word0]].push(word1);
				assigned[word1] = assigned[word0];
			}
			else if(functionrings[assigned[word1]]){
				functionrings[assigned[word1]].push(word0);
	...
	for(i = 0; i < originalringsarray.length; i++) {
		...
		doubles = {}
		targetlookup = 
		assignedvalues[targetword] = target;
		for(j = 0; j < ringcontents.length; j++) {
			assigned[ringcontents[j]] = targetword;
			if(!doubles[ringcontents[j]]){
				doubles[ringcontents[j]] = true;
				trimmedcontents.push(ringcontents[j]);
			}
		}
		sortedrings[targetword] = trimmedcontents
	}	
	...
	for(var i = 0; i < allwordsarray.length; i++){
		...
		assigned0 = assigned[word0]
		assigned1 = assigned[word1]
		if(!absorbedrings[assigned0] || !absorbedrings[assigned1]){
			if(assigned0 != assigned1){
				console.log('combining ' + assigned0 + ' and ' + assigned1)
				//find stronger ring
				if(assignedvalues[assigned0] > assignedvalues[assigned1]){
					movingringassignment = assigned1
					targetringassignment = assigned0
				}
				else{
					movingringassignment = assigned0
					targetringassignment = assigned1
				}
				movingring = sortedrings[movingringassignment]
				for(var j = 0; j < movingring.length; j++){
					sortedrings[targetringassignment].push(movingring[j])
					assigned[movingring[j]] = targetringassignment
				}
				absorbedrings[movingringassignment] = true
				delete sortedrings[movingringassignment]
				console.log(Object.keys(sortedrings).length)
			}
			//otherwise no action needed; already in same ring
		}
		if(Object.keys(sortedrings).length <= maxrings){
			finalrings = Object.keys(sortedrings)
			for(var j = 0; j < finalrings.length; j++){
				console.log(finalrings[j])
				console.log(sortedrings[finalrings[j]])
				console.log('-------------')
			}
			break
		}
