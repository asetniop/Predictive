...
var ringdic = {};
var ringdictionarysize = 15000; // total size of ring stats dictionary
var synonymfloor = 0; 
...
var ringlimit = 10;
var superringlimit = 20; //upper limit of allowable elements before things start getting shunted off into orphans
...
var supercorrelationthreshold = 2000000; //assigns matches to previously built rings if they exceed this value
var nextoutsize = 15;
var outpower = 0.2;
var nextzero = 10;
var slope = 0.03;
...
//GENERATE SYNONYM RINGS
	...
	for(var i = 0; i < allwordsarray.length; i++){
		...
		else{ //if neither is assigned, check for a top-level connection, otherwise start a new ring
			supercorrelationword = word0;
			topcorrelation = supercorrelationthreshold
			word0keys = Object.keys(synonymring[word0])
			for(var j = 0; j < word0keys.length; j++){
				if(rings[word0keys[j]] && synonymring[word0][word0keys[j]] > supercorrelationthreshold){
					topcorrelation = synonymring[word0][word0keys[j]]
					supercorrelationword = word0keys[j]
					break
				}
			}
			word1keys = Object.keys(synonymring[word1])
			for(var j = 0; j < word1keys.length; j++){
				if(rings[word1keys[j]] && synonymring[word1][word1keys[j]] > topcorrelation){
					topcorrelation = synonymring[word1][word1keys[j]]
					supercorrelationword = word1keys[j]
					break
				}
			}
			if(supercorrelationword != word0 && rings[supercorrelationword].length < ringlimit){
				console.log(word0 + word1 + supercorrelationword + topcorrelation)
				assigned[word0] = supercorrelationword;
				assigned[word1] = supercorrelationword;
				rings[supercorrelationword].push(word0);
				rings[supercorrelationword].push(word1);
			}
			else{
